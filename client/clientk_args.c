...
bool validate_cmdargs(cmd_args *process_args, const int argc, const char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i], "-n") == 0)
    {
      ...
      if (clicount < 0)
      {
        fprintf(stderr, "-n requires a positive or zero integer\n");
        return false;
      }
  ...
  if (!hasDLevel)
    fprintf(stderr, "ERROR: -d [dlevel] required a positive integer value\n");
  if (!hasCliCount)
    fprintf(stderr, "ERROR: -n [clicount] requires a positive integer value\n");
  if (!hasVSize)
