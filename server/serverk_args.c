...
bool validate_cmdargs(cmd_args* process_args, const int argc, const char* argv[]) {
  ...
  if (!hasDCount) fprintf(stderr, "ERROR: -d [dcount] required a positive integer value\n");
  if (!hasTCount) fprintf(stderr, "ERROR: -t [tcount] requires a positive integer value\n");
